...
test("maskSensitiveHeaders redacts api-key (Azure OpenAI)", async () => {
  ...
  logger.logTargetRequest(
    "https://myazure.openai.azure.com/openai/deployments/gpt-4o",
    {
  ...
  const headers = payload?.providerRequest?.headers as Record<string, unknown>;
  assert.ok(headers, "providerRequest.headers must exist");
  assert.notEqual(headers["api-key"], "azure-secret-key-67890", "api-key must not appear verbatim");
});
...
test("maskSensitiveHeaders still masks x-api-key (Anthropic)", async () => {
  ...
  const headers = payload?.providerRequest?.headers as Record<string, unknown>;
  assert.ok(headers, "providerRequest.headers must exist");
  assert.notEqual(headers["x-api-key"], "anthropic-key-xyz", "x-api-key must not appear verbatim");
});
