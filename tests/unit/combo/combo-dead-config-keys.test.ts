...
const { DEAD_COMBO_CONFIG_KEYS, stripDeadComboConfigKeys } =
  await import("../../../src/lib/combos/deadConfigKeys.ts");
...
test("exactly 3 dead keys", () => {
  assert.deepEqual(
    ...
    ["pipelineConcurrency", "resetAwareEnabled", "resetAwareWindow"]
  );
});
